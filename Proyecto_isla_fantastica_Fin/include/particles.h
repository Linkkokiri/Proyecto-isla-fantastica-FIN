...
public:
	...
	Particles(unsigned int numParticles) {
		for (unsigned int i = 0; i < numParticles; i++) {
			Particle P;
			P.p0.x = (float)(rand() % 2-0.5); // posición random de -12 a 12rand() % 25-12
			P.p0.y = (float)(rand() % 3);    // posición random de 0 a 7
			P.p0.z = (float)(rand() % 2-0.5); // posición random de -12 a 12
